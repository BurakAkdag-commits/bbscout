...
$player_id = isset($_GET['player_id']) ? intval($_GET['player_id']) :66;
...
<body>
    ...
    echo "<p><strong>Hücum Yeteneği:</strong> Ernst Muci, hücumda oldukça yaratıcı bir oyuncudur. Şut yeteneği güçlü olsa da, 
    en etkili olduğu alanlar pas ve dribbling üzerinden oyun kurma becerisidir. Boş alan yaratma ve takım arkadaşlarını pozisyona sokma 
    konusunda önemli katkılar sağlar. Topla daha fazla etki yaratmak için şutlarını geliştirmesi gerekebilir.</p>";

    echo "<p><strong>İlk Dokunuş ve Baskı Altında Oyun:</strong> Muci, topa ilk dokunuşlarında genellikle başarılıdır ve baskı altındayken 
    sakin kalabilir. Yüksek tempolu oyunlarda soğukkanlılık gösterir ve doğru paslarla takımını yönlendirebilir. Ancak bazen karar verme 
    süreçlerinde daha fazla olgunlaşması gerekebilir.</p>";

    echo "<p><strong>Fiziksel Güç ve Çalışkanlık:</strong> Muci, güçlü bir fiziğe sahip olup, oyun boyunca yüksek bir mücadeleci ruh sergiler. 
    Çevikliği ve dayanıklılığı ile dikkat çeker. Özellikle alan savunmasında ve top kazanma anlarında güçlü performanslar sergiler. 
    Fiziksel gelişimini sürdürüyor ve bu onu daha da etkili bir oyuncu yapacaktır.</p>";

    echo "<p><strong>Pozisyonlar:</strong> Ernst Muci, genellikle hücumda farklı pozisyonlarda görev alabilir. Başlıca pozisyonları şunlardır:</p>";
    echo "<ul>
        <li><strong>Sağ Kanat (7 numara):</strong> Hızlı ve yaratıcı, kanatlarda etkili olabilir.</li>
        <li><strong>Forvet (9 numara):</strong> Ceza sahasında etkili bir bitirici ve takımın hücum hattında güçlü bir oyuncu.</li>
        <li><strong>Forvet Arkası (10 numara):</strong> Oyun kurma yeteneğiyle, takımının oyununu yönlendirebilir ve pozisyon yaratabilir.</li>
    </ul>";

    echo "<p><strong>Savunma Katkısı:</strong> Muci, hücumun yanı sıra savunmaya da katkı sağlar. Rakip hücumları kesmek ve topu geri kazanmak 
    konusunda gayretlidir. Ancak savunma yönünde daha fazla gelişim göstermesi gereken bir oyuncudur.</p>";

    echo "<p><strong>Fiziksel ve Mental Özellikler:</strong></p>";
    echo "<ul>
        <li><strong>Liderlik:</strong> Muci, saha içinde liderlik özellikleri gösteren bir oyuncudur. Hem fiziki hem de mental olarak takıma 
        katkı sağlamaya çalışır.</li>
        <li><strong>Mücadeleci Ruh:</strong> Özellikle zorlu anlarda yüksek motivasyonu ve azmi ile dikkat çeker. Fiziksel gücü ve oyun 
        bilinciyle oyun içinde her an etkilidir.</li>
    </ul>";

    echo "<p><strong>Kariyer Gelişimi ve Potansiyeli:</strong> Ernst Muci, genç yaşta büyük bir potansiyele sahip bir oyuncudur. 
    Takımındaki performansını daha da geliştirebilir ve önümüzdeki yıllarda üst düzey kulüplerde boy gösterme potansiyeline sahiptir. 
    Gelişimini sürdürebilirse, dünya futbolunun önemli oyuncularından biri olabilir.</p>";
...
$main_player_id = 66; // Örneğin, ana oyuncunun ID'si 1 olarak ayarlandı. Dinamik hale getirilebilir.
