...
$player_id = isset($_GET['player_id']) ? intval($_GET['player_id']) :89;
...
echo "<p><strong>Hücum Yeteneği:</strong> Lamine Yamal, hücum yönü güçlü bir oyuncudur ve hücum hattında sergilediği yaratıcılık ile dikkat çeker. Özellikle dar alanlardaki etkili çalımları ve savunmayı aşan paslarıyla fark yaratır. Hücumda birebir pozisyonlarda oldukça başarılıdır ve rakip savunmayı zor durumda bırakabilecek hız ve çevikliğe sahiptir.</p>";

echo "<p><strong>İlk Dokunuş ve Teknik Beceriler:</strong> Yamal, top kontrolü ve ilk dokunuş kalitesiyle öne çıkar. Dar alanlarda rakip baskısından kolayca sıyrılabilir ve topu hücum organizasyonlarında etkin bir şekilde kullanabilir. Çevikliği ve teknik becerileri, pas ve şut seçeneklerini artırır.</p>";

echo "<p><strong>Fiziksel Güç ve Hız:</strong> Lamine Yamal, patlayıcı hız ve çeviklik kombinasyonu sayesinde hücumda dinamik bir tehdit oluşturur. Rakip savunmayı zorlayan sürati ve ani yön değişiklikleri, takımına büyük avantaj sağlar. Yaşına rağmen fiziksel olarak gelişime açık bir oyuncudur.</p>";

echo "<p><strong>Pozisyonlar:</strong> Lamine Yamal, öncelikli olarak sağ kanatta görev yapar. Ancak gerektiğinde sol kanat ve forvet arkası pozisyonlarında da etkili bir şekilde oynayabilir. Çok yönlülüğü, farklı oyun planlarında kullanılabilmesini sağlar.</p>";

echo "<p><strong>Zayıf Yönler:</strong></p>";
echo "<ul>
    <li><strong>Defansif Katkı:</strong> Hücum odaklı bir oyuncu olarak defansif görevlerde zaman zaman eksiklikler gösterebilir. Pozisyon bilgisini ve savunmaya dönüş hızını geliştirmesi gerekebilir.</li>
    <li><strong>Fiziksel Dayanıklılık:</strong> Yaşı gereği, fiziksel dayanıklılığı ve uzun süreli maç temposunu koruma kapasitesi zaman zaman yetersiz kalabilir.</li>
</ul>";

echo "<p><strong>Oyunu Etkileme:</strong> Yamal, yaratıcılığı ve bireysel becerileriyle rakip savunmaları aşabilen bir oyuncudur. Özellikle hücum organizasyonlarında kilit bir rol oynar ve yaptığı asistler ve attığı gollerle skora doğrudan katkıda bulunur.</p>";

echo "<p><strong>Mental Özellikler:</strong></p>";
echo "<ul>
    <li><strong>Özgüven:</strong> Genç yaşına rağmen yüksek özgüveni ve büyük maçlarda bile soğukkanlı performansı dikkat çekicidir.</li>
    <li><strong>Kararlılık:</strong> Sahadaki yüksek kararlılığı ve risk alabilme cesareti, hücumda fark yaratmasını sağlar.</li>
    <li><strong>Takım Oyunu:</strong> Hem bireysel hem de takım odaklı oynayabilen, oyunu iyi okuyarak takım arkadaşlarına fırsatlar yaratabilen bir oyuncudur.</li>
</ul>";

echo "<p><strong>Kariyer Gelişimi ve Potansiyeli:</strong> Lamine Yamal, genç yaşına rağmen yetenekleri, teknik becerileri ve oyun zekasıyla büyük bir potansiyele sahip bir oyuncudur. Düzenli oynama fırsatı bulması ve fiziksel dayanıklılığını artırması halinde, dünya futbolunun en üst seviyesinde kalıcı bir oyuncu olma potansiyeline sahiptir.</p>";
...
$main_player_id = 89; // Örneğin, ana oyuncunun ID'si 1 olarak ayarlandı. Dinamik hale getirilebilir.
...
?>
    ...
    <script>
    const radarData = {
        labels: ['Decision', 'Crossing', 'Contraction', 'Aggresion', 'First Touch', 'Technique', 'Heading', 'Composure', 'Pace', 'Passing', 'Flair', 'Technique'],
        datasets: [
            {
