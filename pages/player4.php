...
$player_id = isset($_GET['player_id']) ? intval($_GET['player_id']) :5;
...
<body>
    <div class="container">
        ...
        <div class="player-report">
            ...
            <p>
<?php
echo "<p><strong>Hücum Yeteneği:</strong> Pedri, Barcelona'nın orta sahasında ofansif oyun kurucu rolünü üstlenen genç bir yetenek. Geçtiğimiz sezon, 38 maçta 9 gol ve 6 asistle takımına önemli hücum katkıları sağladı. Topu çok iyi taşıma, doğru paslar verme ve sahada boşlukları bulma konusunda mükemmel bir vizyona sahip. Özellikle topu alıp hücuma çıkma konusunda çok etkili.</p>";

echo "<p><strong>İlk Dokunuş ve Baskı Altında Oyun:</strong> Pedri'nin top kontrolü olağanüstü. Dar alanlarda bile soğukkanlı kalarak topu mükemmel bir şekilde kontrol edebiliyor ve pres altında hızlı kararlar alabiliyor. Oyun zekası ve soğukkanlılığı, onu baskı altında bile etkili kılıyor.</p>";

echo "<p><strong>Fiziksel Güç ve Çalışkanlık:</strong> Pedri, teknik becerilerinin yanı sıra fiziksel açıdan da oldukça çevik ve dayanıklı bir oyuncu. Koşu yaparken yavaşlamadan topu sürme ve paslaşma yeteneği, onun orta sahada önemli bir oyuncu olmasını sağlıyor. Fiziksel olarak henüz gelişmeye devam etse de oyun zekası sayesinde fazlasıyla etkili bir performans sergiliyor.</p>";

echo "<p><strong>Pozisyonlar:</strong> Pedri, genellikle orta sahada ofansif bir oyun kurucu olarak görev alıyor. Bununla birlikte, zaman zaman sol kanatta da görev alabiliyor ve bu çok yönlülük ona oyun içinde farklı pozisyonlarda katkı sağlama fırsatı tanıyor.</p>";

echo "<p><strong>Zayıf Yönler:</strong></p>";
echo "<ul>
    <li><strong>Fiziksel Dayanıklılık:</strong> Fiziksel olarak çok güçlü olmasa da, zaman zaman rakiplerin sert müdahalelerine karşı savunmasız kalabiliyor. Bu, özellikle yoğun maç programlarında dikkat edilmesi gereken bir nokta.</li>
    <li><strong>Defansif Katkı:</strong> Hücumda oldukça etkili olsa da, savunmada bazen eksik kalabiliyor. Özellikle savunma görevlerinde daha fazla sorumluluk alması bekleniyor.</li>
</ul>";

echo "<p><strong>Savunma Katkısı:</strong> Pedri, genellikle hücumda etkili olsa da, savunmaya da yardımcı olmak için çaba sarf ediyor. Rakiplerin top kaybı yapmalarını engellemek için pres yapıyor, ancak savunma sorumluluğu konusunda zaman zaman zayıf kalabiliyor.</p>";

echo "<p><strong>Fiziksel ve Mental Özellikler:</strong></p>";
echo "<ul>
    <li><strong>Oyun Zekası:</strong> Pedri, sahada inanılmaz bir oyun zekasına sahip. Durumları hızlı bir şekilde analiz edip, doğru pası ve hamleyi yapma konusunda büyük bir yetenek gösteriyor.</li>
    <li><strong>Çalışkanlık:</strong> Sürekli olarak oyunun içinde olan Pedri, maçın temposuna ayak uydurmak için sürekli hareket eder ve takımına enerji verir. Bu, onun güçlü yönlerinden biridir.</li>
</ul>";

echo "<p><strong>Kariyer Gelişimi ve Potansiyeli:</strong> Pedri, genç yaşına rağmen Barcelona'nın merkez oyuncularından biri haline geldi. Onun potansiyeli son derece yüksek ve gelecek yıllarda dünyanın en iyi orta saha oyuncularından biri olma yolunda ilerliyor. Hem teknik hem de mental açıdan gelişmeye devam ettiği sürece, Avrupa'nın en büyük kulüplerinde yerini sağlamlaştırabilir.</p>";
?>
<?php
// Veritabanı bağlantısı
...
$main_player_id = 5; // Örneğin, ana oyuncunun ID'si 1 olarak ayarlandı. Dinamik hale getirilebilir.
...
echo "<tr>
      ...
      if ($similar_players_result->num_rows > 0) {
        while ($row = $similar_players_result->fetch_assoc()) {
            ...
            echo "<tr>
                    <td><a href='player_details.php?player_id={$player_id}'>{$row['player_name']}</a></td>
                    <td>{$row['age']}</td>
                    <td>{$row['positions']}</td>
                  </tr>";
        }
