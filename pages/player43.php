...
$player_id = isset($_GET['player_id']) ? intval($_GET['player_id']) :43;
...
echo "<p><strong>Hücum Yeteneği:</strong> Eduardo Camavinga hücum anlamında doğrudan skor katkısından çok, oyun kurma ve takımını ileri taşımada etkin bir rol oynar. 
Topu taşıma ve hızlı paslarla hücum geçişlerini destekler. Şut tehdidi sınırlı olsa da, hücum setlerini başlatma ve boş alan yaratma konusunda yeteneklidir.</p>";

echo "<p><strong>İlk Dokunuş ve Baskı Altında Oyun:</strong> Camavinga, dar alanlarda bile baskıya dayanıklı bir oyuncudur. 
Top kontrolü ve karar verme hızıyla orta sahada oyunu yönlendirebilir. Baskı altındayken bile sakin kalıp doğru pası bulma konusunda oldukça başarılıdır.</p>";

echo "<p><strong>Fiziksel Güç ve Çalışkanlık:</strong> Camavinga'nın dayanıklılığı, çevikliği ve hızı onun en dikkat çeken özelliklerindendir. 
Uzun mesafelerde hızıyla fark yaratırken, ikili mücadelelerde güçlü fiziğiyle ayakta kalmayı başarır. 
Hem hücum hem de savunmada bitmek bilmeyen bir enerjiyle çalışır.</p>";

echo "<p><strong>Pozisyonlar:</strong> Eduardo Camavinga, modern futbolun çok yönlü oyuncularından biridir. 
Başlıca pozisyonları şunlardır:</p>";
echo "<ul>
    <li><strong>Defansif Orta Saha (6 numara):</strong> Rakip ataklarını kesmede ve top kazanma konusunda başarılıdır.</li>
    <li><strong>Merkez Orta Saha (8 numara):</strong> Topu ileri taşıma ve pas dağıtımında etkilidir.</li>
    <li><strong>Sol Bek:</strong> Real Madrid'de gerektiğinde bu pozisyonda da forma giyip takımına önemli katkılar yapmıştır.</li>
</ul>";

echo "<p><strong>Zayıf Yönler:</strong></p>";
echo "<ul>
    <li><strong>Şut Çekme:</strong> Hücumda uzaktan şut denemeleri genellikle isabetsizdir. Bu konuda kendini geliştirmesi gerekiyor.</li>
    <li><strong>Fiziksel Gelişim:</strong> Daha fazla kas kütlesi kazanarak ikili mücadelelerde daha etkili olabilir.</li>
</ul>";

echo "<p><strong>Savunma Katkısı:</strong> Camavinga, savunma yönüyle ön plana çıkan bir orta saha oyuncusudur. 
Sık sık rakip ataklarını keser, pas arası yapar ve topu geri kazanarak takımına avantaj sağlar. Pozisyon alma yeteneği ve oyun okuma becerisi, 
genç yaşına rağmen oldukça olgundur.</p>";

echo "<p><strong>Fiziksel ve Mental Özellikler:</strong></p>";
echo "<ul>
    <li><strong>Liderlik:</strong> Camavinga, saha içinde lider olma potansiyeli taşır. Henüz genç olmasına rağmen özgüveni ve takım arkadaşlarını motive etme becerisiyle dikkat çeker.</li>
    <li><strong>Mücadeleci Ruh:</strong> Yüksek fiziksel dayanıklılığı ve oyun içinde sürekli maksimum çabayla oynama özelliği, onu modern futbol için ideal bir orta saha oyuncusu yapar.</li>
</ul>";

echo "<p><strong>Kariyer Gelişimi ve Potansiyeli:</strong> Eduardo Camavinga, genç yaşına rağmen hem kulüp hem de milli takım seviyesinde kendini kanıtlamış bir oyuncudur. 
Real Madrid'deki çok yönlü performansları, taktiksel esnekliğini ve futbol zekasını göstermektedir. Gelişimini sürdürdüğü takdirde, dünya futbolunun en üst seviyesinde 
kalıcı bir yer edineceği öngörülmektedir.</p>";
?>
<?php
// Veritabanı bağlantısı
...
$main_player_id = 43; // Örneğin, ana oyuncunun ID'si 1 olarak ayarlandı. Dinamik hale getirilebilir.
