...
$player_id = isset($_GET['player_id']) ? intval($_GET['player_id']) :125;
...
echo "<p><strong>Hücum Yeteneği:</strong> Moisés Caicedo, hücuma verdiği destekle dikkat çeken bir orta saha oyuncusudur. Pas kalitesi ve top taşıma becerisiyle takımının ileri çıkışlarında önemli bir rol oynar. Uzaktan şutları ve ceza sahası çevresindeki paslarıyla etkili olabilir, ancak daha çok topu hücumculara ulaştırma konusunda uzmanlaşmıştır.</p>";

echo "<p><strong>İlk Dokunuş ve Baskı Altında Oyun:</strong> Caicedo, baskı altında soğukkanlı kalarak top kontrolünü kaybetmez. İlk dokunuşundaki hassasiyet ve çeviklik, oyun kurulumunda ve geçiş oyunlarında etkili olmasını sağlar. Özellikle dar alanlarda hızlı düşünüp doğru kararlar alabilir.</p>";

echo "<p><strong>Fiziksel Güç ve Çalışkanlık:</strong> Moisés Caicedo, fiziksel olarak dayanıklı ve atletik bir oyuncudur. Hızı, çevikliği ve fiziksel gücü sayesinde hem savunmada hem de hücumda etkili bir şekilde mücadele eder. Çalışkan yapısıyla 90 dakika boyunca enerjisini dengeli bir şekilde kullanabilir.</p>";

echo "<p><strong>Pozisyonlar:</strong> Caicedo, doğal pozisyonu olan defansif orta sahada mükemmel bir performans sergiler. Ancak box-to-box orta saha rolünde de etkili bir şekilde görev yapabilir. Pozisyon bilgisi ve çok yönlülüğü, farklı taktik dizilişlerde uyum sağlamasına olanak tanır.</p>";

echo "<p><strong>Zayıf Yönler:</strong></p>";
echo "<ul>
    <li><strong>Hücum Katkısı:</strong> Hücumda yaratıcı anlamda zaman zaman sınırlı kalabilir. Daha fazla gol ve asist katkısı sağlayabilmek için bu alanda gelişim göstermesi faydalı olacaktır.</li>
    <li><strong>Hava Topları:</strong> Fiziğine rağmen hava toplarındaki etkinliği sınırlıdır. Hava toplarında daha agresif olması, savunma ve hücumda ona avantaj sağlayabilir.</li>
</ul>";

echo "<p><strong>Savunma Katkısı:</strong> Caicedo, savunmada mükemmel bir duruş sergiler. Top kapma, alan daraltma ve rakip ataklarını kesme konularında uzmanlaşmıştır. Oyunu okuma yeteneği sayesinde pozisyonunu kaybetmez ve genellikle rakiplerin pas yollarını kapatarak takımına savunma güvenliği sağlar.</p>";

echo "<p><strong>Fiziksel ve Mental Özellikler:</strong></p>";
echo "<ul>
    <li><strong>Çalışkanlık:</strong> Sahadaki yüksek enerjisi ve sürekli mücadeleci yapısı, onu takımının vazgeçilmez oyuncularından biri yapar.</li>
    <li><strong>Oyun Okuma:</strong> Savunmadaki pozisyon bilgisi ve oyun okuma yeteneği, orta sahadaki kontrolü sağlamasına yardımcı olur.</li>
    <li><strong>Doğal Fitness:</strong> Yüksek doğal fitness seviyesi, yoğun maç programlarına uyum sağlamasını kolaylaştırır.</li>
</ul>";

echo "<p><strong>Kariyer Gelişimi ve Potansiyeli:</strong> Moisés Caicedo, genç yaşına rağmen Avrupa'nın dikkat çeken orta saha oyuncularından biridir. Defansif yetenekleri, fiziksel yapısı ve oyun zekasıyla modern futbolun gereksinimlerini fazlasıyla karşılamaktadır. Sürekli gelişen yapısıyla, gelecekte dünya futbolunun elit defansif orta saha oyuncuları arasında yer alması beklenmektedir.</p>";
...
$main_player_id = 125; // Örneğin, ana oyuncunun ID'si 1 olarak ayarlandı. Dinamik hale getirilebilir.
