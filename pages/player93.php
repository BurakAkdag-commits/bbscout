...
$player_id = isset($_GET['player_id']) ? intval($_GET['player_id']) :97;
...
echo "<p><strong>Hücum Yeteneği:</strong> William Saliba, bir savunma oyuncusu olmasına rağmen hücuma olan katkılarıyla dikkat çeker. Özellikle topu savunmadan çıkarırken sergilediği soğukkanlılık ve isabetli pasları, takımının hücum organizasyonlarına hızlı bir başlangıç yapmasını sağlar. Duran toplarda hava hakimiyetiyle de rakip ceza sahasında etkili olabilir.</p>";

echo "<p><strong>İlk Dokunuş ve Baskı Altında Oyun:</strong> Saliba, baskı altındayken bile sakin kalmayı başarır ve doğru kararlar verir. Teknik becerileri ve top kontrolü sayesinde rakip forvetlerin presine karşı soğukkanlı bir şekilde oyun kurabilir. İlk dokunuşu, oyun akışını hızlandıran önemli bir unsurdur.</p>";

echo "<p><strong>Fiziksel Güç ve Çalışkanlık:</strong> William Saliba, fiziksel gücü, dayanıklılığı ve çevikliğiyle dikkat çeker. Hızlı ve güçlü yapısı sayesinde hem yerde hem de havada etkili bir savunma performansı sergiler. Maç boyunca enerjisini dengeli bir şekilde kullanarak istikrarlı bir oyun ortaya koyar.</p>";

echo "<p><strong>Pozisyonlar:</strong> Saliba, doğal pozisyonu olan stoperde oynamakla birlikte, gerektiğinde sağ bek pozisyonunda da görev yapabilir. Çok yönlülüğü, farklı taktik sistemlere uyum sağlamasında önemli bir avantajdır ve bu esnekliği takımı için büyük bir artıdır.</p>";

echo "<p><strong>Zayıf Yönler:</strong></p>";
echo "<ul>
    <li><strong>Top Kapma:</strong> Savunmada genel olarak iyi bir performans sergilese de, zaman zaman top kapma anında aceleci kararlar verebiliyor.</li>
    <li><strong>Hücum Katkısı:</strong> Hücumda duran top haricinde doğrudan etkisi sınırlıdır. Oyununu bu alanda geliştirmesi faydalı olabilir.</li>
</ul>";

echo "<p><strong>Savunma Katkısı:</strong> Saliba, savunmadaki sağlam duruşuyla takımının en güvenilir isimlerinden biridir. Rakip atakları kesme, pas arası yapma ve hava toplarındaki üstünlüğüyle savunmaya büyük katkı sağlar. Pozisyon alma becerisi ve oyun okuma yeteneği, onu bir savunma lideri yapmaktadır.</p>";

echo "<p><strong>Fiziksel ve Mental Özellikler:</strong></p>";
echo "<ul>
    <li><strong>Liderlik:</strong> Genç yaşına rağmen sahada bir lider gibi hareket eder ve savunma hattını organize eder.</li>
    <li><strong>Çalışkanlık:</strong> Sürekli kendini geliştirme arzusu ve sahadaki mücadeleci ruhu, onu takımı için vazgeçilmez bir oyuncu yapar.</li>
    <li><strong>Hız:</strong> Rakip hücum oyuncularına karşı hız avantajını kullanarak tehlikeleri savuşturabilir.</li>
</ul>";

echo "<p><strong>Kariyer Gelişimi ve Potansiyeli:</strong> William Saliba, genç yaşında Avrupa'nın en yetenekli savunmacılarından biri olarak gösterilmektedir. Teknik becerileri, fiziksel özellikleri ve mental sağlamlığıyla kısa sürede elit seviyede bir savunmacı haline gelmiştir. Kariyerinde doğru yönlendirme ve gelişimle, dünya futbolunun en üst düzey savunmacılarından biri olması beklenmektedir.</p>";
...
$main_player_id = 97; // Örneğin, ana oyuncunun ID'si 1 olarak ayarlandı. Dinamik hale getirilebilir.
