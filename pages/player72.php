...
$player_id = isset($_GET['player_id']) ? intval($_GET['player_id']) :72;
...
echo "<p><strong>Hücum Yeteneği:</strong> Pau Cubarsí, defansif bir oyuncu olmasına rağmen hücum setlerinde de takımına katkı sağlayabilen bir oyuncudur. Özellikle savunmadan başlattığı oyunlarla takımın hücum organizasyonlarına hız kazandırır. Hava toplarındaki hakimiyeti sayesinde duran toplarda etkili bir silah olabilir.</p>";

echo "<p><strong>İlk Dokunuş ve Baskı Altında Oyun:</strong> Cubarsí, top kontrolü ve oyun görüşüyle dikkat çeker. Baskı altındayken sakin kalabilmesi, savunmadan top çıkarma ve pas isabetiyle takımını rahatlatma konusunda önemli bir rol oynar.</p>";

echo "<p><strong>Fiziksel Güç ve Çalışkanlık:</strong> Pau Cubarsí, güçlü fiziği ve yüksek dayanıklılığı ile tanınır. Rakip forvetlerle ikili mücadelelerde fiziksel üstünlüğünü kullanarak etkili olur. Ayrıca yüksek tempolu maçlarda enerjisini dengeli bir şekilde kullanarak sürekli katkı sağlayabilir.</p>";

echo "<p><strong>Pozisyonlar:</strong> Pau Cubarsí, öncelikli olarak stoper pozisyonunda görev alır. Ancak gerektiğinde bek pozisyonlarında da oynayabilecek çok yönlülüğe sahiptir. Bu esneklik, farklı oyun sistemlerine uyum sağlamasını kolaylaştırır.</p>";

echo "<p><strong>Zayıf Yönler:</strong></p>";
echo "<ul>
    <li><strong>Hücum Katkısı:</strong> Savunma yönü güçlü olsa da, hücuma katkı sağlama konusunda daha fazla gelişime ihtiyaç duyabilir.</li>
    <li><strong>Hız:</strong> Çabuk ve çevik forvetlere karşı zaman zaman hız dezavantajı yaşayabilir.</li>
</ul>";

echo "<p><strong>Savunma Katkısı:</strong> Pau Cubarsí, savunmada pozisyon alma, hava toplarındaki etkinlik ve rakip hücumlarını kesme becerisiyle öne çıkar. Disiplinli savunma anlayışı ve zamanlamasıyla rakip forvetlere göz açtırmaz.</p>";

echo "<p><strong>Fiziksel ve Mental Özellikler:</strong></p>";
echo "<ul>
    <li><strong>Liderlik:</strong> Genç yaşına rağmen savunmada liderlik vasıflarını sergiler ve takım arkadaşlarını yönlendirme becerisine sahiptir.</li>
    <li><strong>Kararlılık:</strong> Sahadaki kararlı oyun yapısı ve zorluklar karşısındaki soğukkanlı duruşu dikkat çeker.</li>
    <li><strong>Takım Oyunu:</strong> Takım savunmasına uyumlu bir oyuncu olarak her zaman iş birliği içinde hareket eder.</li>
</ul>";

echo "<p><strong>Kariyer Gelişimi ve Potansiyeli:</strong> Pau Cubarsí, genç yaşında teknik becerileri, fiziksel özellikleri ve oyun zekasıyla büyük potansiyel vaat eden bir savunma oyuncusudur. Kendini geliştirmeye devam etmesi durumunda, hem kulüp hem de milli takım seviyesinde önemli bir savunma oyuncusu olarak yer alması beklenmektedir.</p>";
...
$main_player_id = 72; // Örneğin, ana oyuncunun ID'si 1 olarak ayarlandı. Dinamik hale getirilebilir.
...
?>
    ...
    <script>
    const radarData = {
        labels: ['Marking', 'Crossing', 'Contraction', 'Aggresion', 'First Touch', 'Technique', 'Heading', 'Composure', 'Long Throws', 'Passing', 'Tackling', 'Technique'],
        datasets: [
            {
